...
from app.XMLupload.constants import point_load_calamity_value
...
class OverviewExcel:
    ...
    def get_struts_table(self) -> List[InputCellRange]:
        ...
        for index, column_letter in enumerate(letter_list):
            ...
            # Add row for fallout struts, if applicable
            fallout_strut = [[data[index][10]]]
            if fallout_strut:
                table.append(
                    InputCellRange(
                        "download stempels", column_letter, 11, data=fallout_strut
                    )
                )
    ...
    def get_control_UGT_strut_cells(self) -> List[DirectInputCell]:
        ...
        return [
            DirectInputCell(
                ...
                "K",
                4,
                self.params.calculation.excentricity,
            ),
            DirectInputCell(
                "Controle blad stempels UGT",
                "K",
                5,
                self.params.calculation.k_value_sheetpile,
            ),
        ]
    ...
    def get_control_BGT_strut_cells(self) -> List[DirectInputCell]:
        ...
        return [
            ...
            DirectInputCell(
                ...
                if self.params.downloads.laden.bgt == "point_load_100"
                else 0,
            ),
        ]
        ...
        for _, strut_row in df_calculation_stamps.iterrows():
            ...
            data.append(
                [
                    ...
                    strut_row.get("fallout_strut", default=""),
                ]
            )
        return data
    ...
    def get_overview_data(self) -> List[List]:
        ...
        # Replace BGT if strut removal
        if self.params.downloads.laden.bgt == "strut_removal":
            for i, overview in enumerate(data):
                for stamp in self.get_stamps_data():
                    if stamp[0] == overview[0]:
                        overview[9] = stamp[9]  # Get BGT normal force from stamps data
