...
class Interface(QMainWindow):
    ...
    def GordingUnityCheck(self):
        ...
        for wall in self.walls:
            for rod in wall:
                rod_dict = properties.fill_in_rod_properties(rod, self.tables)
                print(rod_dict)
                rod_dict["UGT Normaalkracht"] = 563.88
                rod_dict["UGT Dwarskracht"] = 561.75
                uc_items = {k: v for k, v in rod_dict.items() if "u.c" in k}
                for key, value in uc_items.items():
                    print(f"{key} → {value()}")
                exit()
